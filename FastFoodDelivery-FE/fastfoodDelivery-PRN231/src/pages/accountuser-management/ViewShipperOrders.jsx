...
import { fetchShipperOrders, updateOrderDeliveryStatus } from '../../redux/features/ViewShipperOrders';
...
const ViewShipperOrders = () => {
  ...
  const confirmAction = (orderId, action) => {
    Modal.confirm({
      title: `Are you sure you want to ${action === 'confirm' ? 'confirm' : 'cancel'} this order?`,
      onOk() {
        action === 'confirm' ? handleConfirmOrder(orderId) : handleCancelOrder(orderId);
      }
    });
  };
  ...
  const handleConfirmOrder = async (orderId) => {
    try {
      const response = await axios.delete(`https://localhost:7173/api/Orders/GetConfirmOrderByShipper/${orderId}`);
  ...
  const handleCancelOrder = async (orderId) => {
    try {
      const response = await axios.delete(`https://localhost:7173/api/Orders/GetCancelOrderByShipper/${orderId}`);
      if (response.status === 200) {
  ...
  if (!orders.length) {
    return <div className="container mt-3"><h2>Shipper Orders</h2><p>No orders to view.</p></div>;
  }
