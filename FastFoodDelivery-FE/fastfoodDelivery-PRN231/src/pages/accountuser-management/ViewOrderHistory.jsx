...
import { fetchOrderHistory, updateOrderStatus } from '../../redux/features/ViewOrderHistory';
...
const ViewOrderHistory = () => {
  ...
  const fetchFeedbackStatus = async (userId) => {
    try {
      const response = await axios.get(`https://localhost:7173/api/FeedBacks/ViewAllFeedBackByUserID/${userId}`);
  ...
  const handleConfirmOrder = async (orderId) => {
    Modal.confirm({
      ...
      onOk: async () => {
        try {
          const response = await axios.delete(`https://localhost:7173/api/Orders/GetConfirmOrderByUser/${orderId}`);
    ...
    Modal.confirm({
      ...
      content: (
        <Form onFinish={async (values) => {
          try {
            const response = await axios.post('https://localhost:7173/api/FeedBacks/CreateFeedBack', {
              userId: UserId,
              orderId: orderId,
  ...
  const handleViewFeedback = async (orderId) => {
    const response = await axios.get(`https://localhost:7173/api/FeedBacks/ViewAllFeedBackByUserID/${UserId}`);
    const feedback = response.data.data.find(feedback => feedback.orderId === orderId);
      ...
      Modal.info({
        ...
        content: (
          <div>
            ...
            <Button onClick={() => handleUpdateFeedback(feedback.feedBackId)}>Update</Button>
            <Button onClick={() => handleDeleteFeedback(feedback.feedBackId, orderId)}>Delete</Button>
  ...
  const handleUpdateFeedback = async (feedbackId) => {
    Modal.confirm({
      ...
      content: (
        <Form onFinish={async (values) => {
          try {
            const response = await axios.put(`https://localhost:7173/api/FeedBacks/UpdateFeedBack/${feedbackId}`, {
              commentMsg: values.comment
            });
            Modal.destroyAll();
          } catch (error) {
          ...
          <Form.Item name="comment" rules={[{ required: true, message: 'Please input your updated comment!' }]}>
  ...
  const handleDeleteFeedback = async (feedbackId, orderId) => {
    Modal.confirm({
      ...
      onOk: async () => {
        try {
          const response = await axios.delete(`https://localhost:7173/api/FeedBacks/DeleteFeedBack/${feedbackId}`);
            ...
            <tr key={order.orderId}>
              ...
              <td>
                ...
                <Button
                  ...
                  disabled={order.deliveryStatus !== 'Received' || feedbackStatus[order.orderId]}
                >
                  Feedback
                </Button>
                <Button
